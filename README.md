...
直接使用发布包通常不需要额外安装 .NET；从源码运行或二次开发需要安装 .NET 8 SDK。
...
5. 如需打印后退款报警，请保持已登录的批量打印页面打开，并把每页数量设置为能够覆盖目标订单且不超过 200 条。脚本会在后台打开一个不抢焦点的“退款核验工作页”，只有该专用页面会切换“打印后退款”筛选并回传数据，用户正在操作的打印页面不会被切换。请勿关闭工作页；误关后脚本会按冷却时间自动重建，也可从扩展菜单手动重新打开。
